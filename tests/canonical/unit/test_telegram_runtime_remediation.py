...
def test_unauthorized_and_rate_limited_callbacks_do_not_accumulate_messages(
    ...
    # Unauthorized ADMIN_NAV: callbacks now return a toast ack text instead of
    # editing or sending a message — the navigation UI is not overwritten.
    ...
    # Rate-limited callback: still produces an edit (not a toast)
...
def test_non_owner_callback_navigation_requires_admin_topic_thread(
    ...
    # Unauthorized ADMIN_NAV: callbacks now return ack text without touching the
    # navigation message — no new send, no edit.
    assert sends == []
