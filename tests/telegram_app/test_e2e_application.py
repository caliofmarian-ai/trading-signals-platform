...
# ---------------------------------------------------------------------------
# Test: Edit failure classification for idempotent APP navigation
# ---------------------------------------------------------------------------

class TestAppNavEditFailureClassification:
    def _run_scenario(self, roles_config_file, *, send_message_ids, edit_side_effect, scenario):
        import importlib, sys
        ...
        with patch.dict(os.environ, {"ADMIN_ROLES_CONFIG": roles_config_file, "SHADOW_MODE": "false",
            ...
            sends = []
            edits = []
            queue = list(send_message_ids)

            def _fake_send(*a, **kw):
                message_id = queue.pop(0) if queue else 99999
                sends.append({"args": a, "kwargs": kw, "message_id": message_id})
                return {"result": {"message_id": message_id}}

            def _fake_edit(*a, **kw):
                edits.append({"args": a, "kwargs": kw})
                effect = edit_side_effect(a, kw, len(edits))
            ...
            with patch.object(importlib.import_module("core.telegram_publisher"), "send_message", _fake_send), \
                 patch.object(importlib.import_module("core.telegram_publisher"), "edit_message", _fake_edit):
                scenario(bot, app_nav, sends, edits)

            return app_nav, sends, edits

    def test_repeated_status_not_modified_is_noop_and_keeps_active_tracking(self, roles_config_file):
        error = RuntimeError(
            "Telegram edit_message failed: {'ok': False, 'error_code': 400, "
            "'description': 'Bad Request: message is not modified'}"
        )

        def _edit_side_effect(_args, _kwargs, _n):
            return error

        def _scenario(bot, _app_nav, _sends, _edits):
            bot.process_update(_message_update(chat_id=9999, user_id=9999, text="/status", message_id=100))
            bot.process_update(_message_update(chat_id=9999, user_id=9999, text="/status", message_id=101))

        app_nav, sends, edits = self._run_scenario(
            roles_config_file,
            send_message_ids=[5001],
            edit_side_effect=_edit_side_effect,
            scenario=_scenario,
        )

        assert len(sends) == 1
        assert len(edits) == 1
        assert app_nav.get_active_message(user_id=9999, chat_id=9999) == 5001

    def test_repeated_identical_status_callback_not_modified_does_not_send_duplicate(self, roles_config_file):
        error = RuntimeError(
            "Telegram edit_message failed: {'ok': False, 'error_code': 400, "
            "'description': 'Bad Request: message is not modified'}"
        )
        ...
        def _scenario(bot, _app_nav, _sends, _edits):
            bot.process_update(_message_update(chat_id=9999, user_id=9999, text="/status"))
            bot.process_update(_callback_update(chat_id=9999, user_id=9999, data="APP:STATUS", message_id=7001))
            bot.process_update(_callback_update(chat_id=9999, user_id=9999, data="APP:STATUS", message_id=7001))

        app_nav, sends, edits = self._run_scenario(
            roles_config_file,
            send_message_ids=[7001],
            edit_side_effect=_edit_side_effect,
            scenario=_scenario,
        )

        assert len(sends) == 1
        assert len(edits) == 4  # 2 callback direct edits + 2 tracked-active retries
        assert app_nav.get_active_message(user_id=9999, chat_id=9999) == 7001

    def test_duplicate_rapid_status_requests_remain_single_message(self, roles_config_file):
        error = RuntimeError(
            "Telegram edit_message failed: {'ok': False, 'error_code': 400, "
            "'description': 'Bad Request: message is not modified'}"
        )
        ...
        def _scenario(bot, _app_nav, _sends, _edits):
            for msg_id in range(100, 106):
                bot.process_update(_message_update(chat_id=9999, user_id=9999, text="/status", message_id=msg_id))

        app_nav, sends, edits = self._run_scenario(
            roles_config_file,
            send_message_ids=[8001],
            edit_side_effect=_edit_side_effect,
            scenario=_scenario,
        )
        ...
        assert len(sends) == 1
        assert len(edits) == 5
        assert app_nav.get_active_message(user_id=9999, chat_id=9999) == 8001

    def test_stale_active_message_sends_exactly_one_replacement(self, roles_config_file):
        ...
        assert len(edits) == 1
        assert len(sends) == 2
        assert app_nav.get_active_message(user_id=9999, chat_id=9999) == 9002
