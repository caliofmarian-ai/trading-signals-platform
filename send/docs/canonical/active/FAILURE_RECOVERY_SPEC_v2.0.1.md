...
This successor preserves all recovery-state, restoration, backup, dedup, degraded-mode, idempotency and fail-safe semantics of v2.0.0.
...
This document is active under `CANONICAL_MASTER_INDEX_v2.0.0.md` and the executed `CANONICAL_ACTIVATION_RECORD_20260901.md`. `FAILURE_RECOVERY_SPEC_v2.0.0.md` is superseded and historical only. Documentation activation does not itself authorize runtime changes.
...
- use backup fallback when primary state is corrupted

Those foundations remain correct.

The v2 model strengthened the legacy model through:
- route-governance terminology
- explicit degraded-mode governance
- stronger recovery event semantics
- invariant-breach escalation
- admin/control implications during impaired operation
- alignment to the canonical document graph

This v2.0.1 successor preserves those safeguards while repairing references to the promotion graph.
...
- signal scoring or Trade Physics logic
- Telegram UX copy
...
- invariant-breach severity reaches a level defined by `SYSTEM_INVARIANTS_v3.0.0.md`

The refusal-to-start behavior on severe corruption remains canonical.
...
At minimum, observability must support the canonical recovery/failure event families defined by `EVENT_SCHEMA_SPEC_v3.0.0.md` and `OBSERVABILITY_LOGGING_SPEC_v3.0.0.md`, including recovery start/completion, dependency degradation, configuration failure, warning/error and invariant-breach evidence where applicable.
...
Failure and recovery behavior must remain subordinate to `SYSTEM_INVARIANTS_v3.0.0.md`.
...
Canonical recovery behavior should be validated under `TEST_PLAN_v3.0.0.md` against scenarios such as:
- clean restart
...
- cleaner linkage to the modern canonical stack
...
|---|---|---|
| 2.0.1 | 2026-09-01 | Proposed PATCH successor for canonical reference repair only; failure/recovery semantics unchanged. |
| 2.0.0 | 2026-07-12 | Active canonical recovery model before this proposed patch. |

---

End of FAILURE_RECOVERY_SPEC_v2.0.1.md
