...
This successor preserves the complete human-comprehension, operational-memory, explanation, evidence-gap, truth-separation and acceptance contract of v1.0.0.
...
When the required evidence is absent, invalid, or unreadable, the surface must display `UNKNOWN`, `UNAVAILABLE`, or an equally explicit evidence-gap state. It must not substitute a healthy, ready, enabled, disabled, profitable, or otherwise reassuring value.

Stable definitions and explanations are canonical knowledge data. They must be maintained in a versioned declarative registry and validated by renderer code, not duplicated as inline operational content across Python handlers. Interface labels and rendering structure are not operational observations.
...
11. current-state values are traceable to runtime, configuration, persisted evidence, or an explicit derivation;
12. missing evidence produces an explicit evidence-gap state rather than a hardcoded operational value;
13. canonical explanatory content is loaded from a versioned declarative registry rather than embedded across renderer implementations.
...
End of HUMAN_COMPREHENSION_AND_SELF_EXPLAINING_CONTROL_SURFACE_CANON_v1.0.1.md
