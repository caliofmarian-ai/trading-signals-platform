...
- this document is the detailed mathematical authority for Trade Physics metric derivation and deterministic TPS.
...
The intake sources contain overlapping and conflicting definitions. This canon resolves them as follows.
...
A learned/calibrated model output is named `trade_success_probability` in this canon.
...
### 7.1 v1 deterministic algorithm

This section is a technical reconciliation introduced by this canonical successor because the intake source defines the concept but does not provide exact weights.
...
Trade Physics is current-scope and mandatory in the active strategic contract. Runtime decision influence still requires governed implementation alignment and validation.
...
A Trade Physics evaluation must expose one of the following states:
...
Any future adjustment to the exact state enum requires governed canonical change; missing evidence MUST remain explicit.

---

## 15. DECISIONOBJECT CONTRACT

The active strategic contract must expose a recognizable Trade Physics domain or score subdomain containing at minimum:
...
This canon treats those as approved research candidates, not as a locked library dependency.
...
The following deterministic defaults are structural model constants in this v1 contract:
...
Canonical activation does not itself authorize runtime code changes.
...
## 26. CURRENT STATUS AND IMPLEMENTATION GATE

Status now: ACTIVE CANONICAL.

The executed canonical promotion reconciled the authority graph required for this document:

- ALGO defines TPS current decision role;
- SR/Corridor exposes the required structural inputs;
- Time Model formalizes Trade Physics time consumption without duplicate time authority;
- DecisionObject carries Trade Physics evidence;
- observability/event schema and analytics/intelligence contracts are reconciled;
- the prior future-state classification is superseded by the executed authority graph;
- Root Stack and Master Index identify this document as active.

Canonical activation does not itself authorize runtime code modification. Implementation changes remain subject to governed audit, tests, deployment discipline, rollback and runtime acceptance.
