...
This successor preserves the complete deployment classes, evidence, backup, pre/post scan, restart, rollback, state/config protection and success/failure semantics of v2.0.0.
...
This document is active under `CANONICAL_MASTER_INDEX_v2.0.0.md` and the executed `CANONICAL_ACTIVATION_RECORD_20260901.md`. `DEPLOYMENT_PROTOCOL_v2.0.0.md` is superseded and historical only. Documentation activation does not itself authorize deployment or runtime changes.
...
This document does not define trading logic, Trade Physics mathematics, UX wording, or signal scoring rules themselves. It defines how approved system changes are introduced into runtime safely.
...
- deterministic Trade Physics contract introduction or comparable strategy-contract redesign
...
1. The relevant active canonical documents already describe the intended system truth.
...
When the change affects architecture, contracts, governance, state ownership, deployment protocol, observability semantics, distribution policy, strategy/Trade Physics contracts, or other canonical behavior, documentation must be aligned before code implementation.
...
- deprecated/superseded move incomplete where applicable
...
- supersede/deprecate a legacy doc
- patch a command
...
- correct active/superseded/deprecated folder placement
- explicit superseded timestamps or records where required
...
## 21. Active/Superseded Transition Rule

When an active or legacy document is replaced by a canonical successor:
1. the successor must exist correctly and have the required approval/promotion basis
2. the predecessor must be backed up or otherwise recoverable under repository history
3. active placement and superseded placement must be changed in a controlled sequence
4. the move must be proven in output_docs or equivalent canonical promotion evidence
5. post-checks must confirm one active version per domain and the predecessor no longer competes as active truth

No supersession step is valid if successor placement or authority is still uncertain.
...
This relationship must align with `GOVERNANCE_AND_CHANGE_CONTROL_v2.0.1.md` and `TEST_PLAN_v3.0.0.md`.
...
This relationship must align with `OBSERVABILITY_LOGGING_SPEC_v3.0.0.md` and `FAILURE_RECOVERY_SPEC_v2.0.1.md`.
...
|---|---|---|
| 2.0.1 | 2026-09-01 | Proposed PATCH successor for canonical reference repair only; deployment semantics unchanged. |
| 2.0.0 | 2026-07-12 | Active canonical deployment protocol before this proposed patch. |

---

End of DEPLOYMENT_PROTOCOL_v2.0.1.md
